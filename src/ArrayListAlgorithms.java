...
public class ArrayListAlgorithms
{
    ...
    public static int indexOfMinimum(ArrayList<Integer> intList)
    {
        ...
        {
            if(intList.get(i) < intList.get(min_index))
            {
                min_index = i;
            }
        }
    ...
    {
        String[] banned_vowels = {"a", "e", "i", "o"};
        for(int i = 0; i < wordList.size(); i++)
        {
            for(String banned_letter : banned_vowels)
            {
                if (wordList.get(i).indexOf(banned_letter) != -1)
                {
                    wordList.remove(i);
                    i--;
                    break;
    ...
    {
        ArrayList<Integer> removedInt = new ArrayList<>();
        for(int i = 0; i < intList.size(); i++)
        {
            if (removedInt.contains(intList.get(i)))
            {
                for(int removed: removedInt)
                {
                    if (intList.get(i) == removed)
                    {
                        intList.remove(i);
                        i--;
                        break;
    ...
    public static void moveBWords(ArrayList<String> wordList)
    {
        ...
        for(int i = 0; i < wordList.size(); i++)
        {
            ...
            {
                wordList.add(indexOfLastB+1,wordList.remove(i));
                indexOfLastB++;
                i--;
            }
        }
